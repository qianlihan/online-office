...
//    private String code;
    ...
    @Override
    public Integer login(HashMap userPassword) {
        return userDao.login(userPassword);
    }

    @Override
    public Integer updatePassword(HashMap userIdPassword) {
        return userDao.updatePassword(userIdPassword);
    }

    @Override
    public PageUtils searchUserByPage(HashMap param) {
        ArrayList<HashMap> pageResult = userDao.searchUserByPage(param);
        long count = userDao.searchUserCount(param);
        int start = (Integer) param.get("start");
        int length = (Integer) param.get("length");
        return new PageUtils(pageResult, count, start, length);
    }
    ...
    private String getOpenId(String code) {
        ...
        if (openId == null || openId.length() == 0) {
            throw new RuntimeException("temporary loging error");
        }
        return openId;
    }
}
